...
Dial.prototype.createText = function () {
    ...
    text.innerHTML = 0 + '<tspan font-size="' + tspanSize + '" dy="' + -tspanSize * 1.2 + '">' + unit + '</tspan>';
...
Dial.prototype.animateTo = function (value) {
    var v = 0;
    var self = this;
...
Dial.prototype.setValue = function (value) {
    ...
    this.text.innerHTML = Math.floor(value) + '<tspan font-size="' + tspanSize + '" dy="' + -tspanSize * 1.2 + '">' + unit + '</tspan>';
};
...
document.addEventListener("DOMContentLoaded", () => {
    for (const chart of document.getElementsByClassName("chart")) {
        const dial = new Dial(chart);
        setTimeout(()=>{
            dial.animateTo(Math.random()*100)
        }, 500)
    }
})